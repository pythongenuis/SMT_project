...
if str(line) == "sat":
    ...
    for L in range(1, len (list_strip_not_existant_element) + 1):
        for subset in itertools.combinations (list_strip_not_existant_element, L):
            ...
            print("HHHHHvoilà à nouveau les prpriété du nouveau état"+ str(new_state_provisoire))
            ...
            print("Et vooila celle que je suis sencé injecté:"+ str(str_file_smt_input_with_new_smt))
            mon_fichier = open ("fichier.smt", "w")
            mon_fichier.write ("(set-logic ALL)\n")
            mon_fichier.write ("(set-option :produce-models true)\n")
            i = 0
            ...
            if str(line) == "sat":
                ...
                print("Cette combinaisons de fonctions de sécurité ne peut pas sécuriser le composant:")
                for el in subset:
                    print("not " + str (el))
            mon_fichier.close()
